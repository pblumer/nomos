...
var apiEndpoints = []apiEndpoint{
	...
	{Method: "POST", Path: "/api/v1/repositories", Tag: "Repositories", Summary: "Create or open repository", Description: "Without a location, scaffolds a new git-first repository (create new). With a location, attaches an existing directory on the server's filesystem without scaffolding files (open / mount an existing repository).", Request: object(map[string]any{"name": stringSchema("Display name (optional when opening; defaults to the directory base name)."), "location": stringSchema("Existing directory to attach, absolute or workspace-relative. Omit to create a new repository.")}), Response: schemaRef("Repository"), Code: "201"},
	...
	{Method: "PATCH", Path: "/api/v1/repositories/{repo}", Tag: "Repositories", Summary: "Rename repository", Description: "Updates a repository's display name. The default repository is renamed via its cosmos.yaml.", Request: object(map[string]any{"name": stringSchema("New display name.")}), Response: schemaRef("Repository")},
	{Method: "DELETE", Path: "/api/v1/repositories/{repo}", Tag: "Repositories", Summary: "Delete repository", Description: "Removes a filesystem repository and its working directory. The default repository cannot be deleted.", Response: schemaRef("DeletedResponse"), Code: "204"},
	{Method: "POST", Path: "/api/v1/repositories/{repo}/fs/delete", Tag: "Repositories", Summary: "Delete file or folder", Description: "Removes the file or folder at the given repository-relative path (folders recursively).", Request: object(map[string]any{"path": stringSchema("Repository-relative path to delete.")}), Response: schemaRef("DeletedResponse")},
	{Method: "POST", Path: "/api/v1/repositories/{repo}/fs/rename", Tag: "Repositories", Summary: "Rename file or folder", Description: "Renames the file or folder at path to a new base name in the same parent.", Request: object(map[string]any{"path": stringSchema("Repository-relative path."), "name": stringSchema("New base name.")}), Response: schemaRef("DeletedResponse")},
	{Method: "GET", Path: "/api/v1/repositories/{repo}/git/status", Tag: "Repositories", Summary: "Git working-tree status", Description: "Returns branch, HEAD and the list of uncommitted changes (ADR-0022 §3, local git-first).", Response: schemaRef("GitStatus")},
	{Method: "POST", Path: "/api/v1/repositories/{repo}/git/commit", Tag: "Repositories", Summary: "Commit changes", Description: "Stages and commits all working-tree changes. Initializes git on first commit for a fresh repository.", Request: object(map[string]any{"message": stringSchema("Commit message.")}), Response: schemaRef("GitStatus")},
	{Method: "GET", Path: "/api/v1/repositories/{repo}/git/branches", Tag: "Repositories", Summary: "List branches", Description: "Returns the local branches and the current branch.", Response: schemaRef("GitBranches")},
	{Method: "POST", Path: "/api/v1/repositories/{repo}/git/branches", Tag: "Repositories", Summary: "Create branch", Description: "Creates a branch, optionally switching to it.", Request: object(map[string]any{"name": stringSchema("Branch name."), "checkout": map[string]any{"type": "boolean", "description": "Switch to the new branch."}}), Response: schemaRef("GitBranches")},
	{Method: "POST", Path: "/api/v1/repositories/{repo}/git/checkout", Tag: "Repositories", Summary: "Switch branch", Description: "Switches the working tree to an existing branch.", Request: object(map[string]any{"branch": stringSchema("Branch to switch to.")}), Response: schemaRef("GitBranches")},
	{Method: "GET", Path: "/api/v1/repositories/{repo}/git/tags", Tag: "Repositories", Summary: "List tags", Description: "Returns the repository's tags (releases are modeled as tags).", Response: schemaRef("GitTags")},
	{Method: "POST", Path: "/api/v1/repositories/{repo}/git/tags", Tag: "Repositories", Summary: "Create tag", Description: "Creates a tag (release) at HEAD; a non-empty message produces an annotated tag.", Request: object(map[string]any{"name": stringSchema("Tag name."), "message": stringSchema("Optional annotation message.")}), Response: schemaRef("GitTags")},

	// Types (user-defined artifact types and their instances)
	{Method: "GET", Path: "/api/v1/repositories/{repo}/types", Tag: "Types", Summary: "List type definitions", Description: "Returns the user-defined types under .nomos/types.", Response: genObj()},
	{Method: "POST", Path: "/api/v1/repositories/{repo}/types", Tag: "Types", Summary: "Create type definition", Description: "Creates a user-defined type. The id must be a lowercase slug.", Request: genObj(), Response: genObj(), Code: "201"},
	{Method: "GET", Path: "/api/v1/repositories/{repo}/types/{type}", Tag: "Types", Summary: "Get type definition", Description: "Returns one user-defined type by id.", Response: genObj()},
	{Method: "PUT", Path: "/api/v1/repositories/{repo}/types/{type}", Tag: "Types", Summary: "Update type definition", Description: "Creates or replaces a user-defined type; the id is taken from the path.", Request: genObj(), Response: genObj()},
	{Method: "DELETE", Path: "/api/v1/repositories/{repo}/types/{type}", Tag: "Types", Summary: "Delete type definition", Description: "Removes a user-defined type and its data-entry form.", Response: schemaRef("DeletedResponse"), Code: "204"},
	{Method: "GET", Path: "/api/v1/repositories/{repo}/types/{type}/instances", Tag: "Types", Summary: "List or read type instances", Description: "Without ?path, lists every directory holding the type's detection file. With ?path=<dir>, returns that single instance.", Response: genObj()},
	{Method: "POST", Path: "/api/v1/repositories/{repo}/types/{type}/instances", Tag: "Types", Summary: "Create type instance", Description: "Creates a directory at path and writes the type's detection file with the supplied data.", Request: object(map[string]any{"path": stringSchema("Repository-relative directory for the instance."), "data": genObj()}), Response: genObj(), Code: "201"},
...
func schemas() map[string]any {
	return map[string]any{
		...
		"GitFileChange":              object(map[string]any{"code": stringSchema("Porcelain status code, e.g. M, A, D, ??."), "path": stringSchema("Changed path.")}),
		"GitStatus":                  object(map[string]any{"initialized": map[string]any{"type": "boolean"}, "branch": stringSchema("Current branch."), "head": stringSchema("Short HEAD hash."), "dirty": map[string]any{"type": "boolean"}, "files": arrayOf(schemaRef("GitFileChange"))}),
		"GitBranches":                object(map[string]any{"current": stringSchema("Current branch."), "branches": arrayOf(map[string]any{"type": "string"})}),
		"GitTag":                     object(map[string]any{"name": stringSchema("Tag name."), "message": stringSchema("Annotation message.")}),
		"GitTags":                    object(map[string]any{"tags": arrayOf(schemaRef("GitTag"))}),

...
def test_product_detail_endpoint_returns_single_product(monkeypatch, tmp_path: Path) -> None:
    products_dir = tmp_path / "products"
    products_dir.mkdir()
    _write_product(products_dir)
...
def test_product_detail_endpoint_returns_404_when_missing(monkeypatch, tmp_path: Path) -> None:
    products_dir = tmp_path / "products"
    products_dir.mkdir()
    _write_product(products_dir)
    ...
    response = client.get("/api/v1/products/unbekannt")

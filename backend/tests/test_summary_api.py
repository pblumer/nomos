...
def test_summary_endpoint_returns_aggregated_counts(monkeypatch, tmp_path: Path) -> None:
    ...
    response = client.get("/api/v1/products/produkt-1/summary")
...
def test_summary_endpoint_returns_404_for_unknown_product(monkeypatch, tmp_path: Path) -> None:
    products_dir = tmp_path / "products"
    products_dir.mkdir()
    _write_product(products_dir)
    ...
    response = client.get("/api/v1/products/unknown/summary")

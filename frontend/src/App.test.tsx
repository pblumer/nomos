...
describe("App", () => {
  it("shows Nomos title", () => {
    ...
    render(<App />);
    expect(screen.getByRole("heading", { name: "Nomos" })).toBeInTheDocument();
  });
  ...
  it("loads and renders product names from API", async () => {
    ...
    vi.spyOn(window, "confirm").mockReturnValue(true);
    render(<App />);
    fireEvent.click(screen.getByRole("button", { name: "Product Catalog" }));
  ...
  it("loads product detail when a product is clicked", async () => {
    ...
    render(<App />);
    fireEvent.click(screen.getByRole("button", { name: "Product Catalog" }));
    fireEvent.click(await screen.findByRole("button", { name: "Open product Benutzerkonto mit Mailbox" }));
  ...
  it("shows validation errors in product detail", async () => {
      ...
      if (url.includes("/api/v1/products/produkt-invalid")) {
        return {
          ...
          json: async () => ({
            ...
            validation: { is_valid: false, errors: ["Missing required field: version"] },
          }),
        } as Response;
      }
      return {
        ok: true,
        json: async () => ({
          items: [{ id: "produkt-invalid", name: "Broken Product", version: "" }],
          count: 1,
        }),
      } as Response;
    });
    ...
    render(<App />);
    fireEvent.click(screen.getByRole("button", { name: "Product Catalog" }));
    fireEvent.click(await screen.findByRole("button", { name: "Open product Broken Product" }));
  ...
  it("loads requirements and supports filtering", async () => {
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
      ...
      if (url.includes("/api/v1/products/produkt-1/requirements")) {
        return {
          ok: true,
          json: async () => ({ product_id: "produkt-1", items: ["mailbox-enabled", "login-required", "account-owner-present"], count: 3 }),
        } as Response;
      }
      if (url.includes("/api/v1/products/produkt-1")) {
        return {
          ok: true,
          json: async () => ({ id: "produkt-1", name: "Benutzerkonto mit Mailbox", version: "0.1.0", description: "Reference product for Nomos MVP", validation: { is_valid: true, errors: [] } }),
        } as Response;
      }
      return {
        ok: true,
        json: async () => ({ items: [{ id: "produkt-1", name: "Benutzerkonto mit Mailbox", version: "0.1.0" }], count: 1 }),
      } as Response;
    });
    vi.spyOn(window, "confirm").mockReturnValue(true);
    render(<App />);
    ...
    fireEvent.change(screen.getByPlaceholderText("Filter requirements"), { target: { value: "login" } });
  ...
  it("supports rules tab with filtering and sorting", async () => {
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
      ...
      if (url.includes("/api/v1/products/produkt-1/rules")) {
        return { ok: true, json: async () => ({ product_id: "produkt-1", items: ["rule-zeta", "rule-alpha", "rule-gamma"], count: 3 }) } as Response;
      }
      if (url.includes("/api/v1/products/produkt-1")) {
        return { ok: true, json: async () => ({ id: "produkt-1", name: "Benutzerkonto mit Mailbox", version: "0.1.0", description: "Reference product for Nomos MVP", validation: { is_valid: true, errors: [] } }) } as Response;
      }
      return { ok: true, json: async () => ({ items: [{ id: "produkt-1", name: "Benutzerkonto mit Mailbox", version: "0.1.0" }], count: 1 }) } as Response;
    });
    vi.spyOn(window, "confirm").mockReturnValue(true);
    render(<App />);
    fireEvent.click(screen.getByRole("button", { name: "Product Catalog" }));
    ...
    fireEvent.change(screen.getByPlaceholderText("Filter rules"), { target: { value: "alpha" } });
  ...
  it("shows summary cards in product detail header", async () => {
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
      ...
      if (url.includes("/api/v1/products/produkt-1/summary")) {
        return { ok: true, json: async () => ({ product_id: "produkt-1", name: "Benutzerkonto mit Mailbox", version: "0.1.0", requirements_count: 2, rules_count: 3, validation_is_valid: true, validation_error_count: 0 }) } as Response;
      }
      if (url.includes("/api/v1/products/produkt-1")) {
        return { ok: true, json: async () => ({ id: "produkt-1", name: "Benutzerkonto mit Mailbox", version: "0.1.0", description: "Reference product for Nomos MVP", validation: { is_valid: true, errors: [] } }) } as Response;
      }
  ...
  it("allows adding and removing requirements and rules", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
      ...
      if (url.includes("/api/v1/products/produkt-1/summary")) {
        return { ok: true, json: async () => ({ product_id: "produkt-1", name: "Benutzerkonto mit Mailbox", version: "0.1.0", requirements_count: requirements.length, rules_count: rules.length, validation_is_valid: true, validation_error_count: 0 }) } as Response;
      }
      if (url.includes("/api/v1/products/produkt-1/requirements") && method === "GET") {
        return { ok: true, json: async () => ({ product_id: "produkt-1", items: [...requirements], count: requirements.length }) } as Response;
      }
      ...
      if (url.includes("/api/v1/products/produkt-1/rules") && method === "GET") {
        return { ok: true, json: async () => ({ product_id: "produkt-1", items: [...rules], count: rules.length }) } as Response;
      }
  ...
  it("shows success and error toasts for requirement actions", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
      ...
      const method = init?.method ?? "GET";
      if (url.includes("/api/v1/products/produkt-1/summary")) {
        return { ok: true, json: async () => ({ product_id: "produkt-1", name: "Benutzerkonto mit Mailbox", version: "0.1.0", requirements_count: requirements.length, rules_count: 2, validation_is_valid: true, validation_error_count: 0 }) } as Response;
      }
      if (url.includes("/api/v1/products/produkt-1/requirements") && method === "GET") {
        return { ok: true, json: async () => ({ product_id: "produkt-1", items: [...requirements], count: requirements.length }) } as Response;
      }
      if (url.includes("/api/v1/products/produkt-1/requirements") && method === "POST") {
        ...
        if (body.value === "login-required") {
          return { ok: false, status: 409, json: async () => ({ detail: "item already exists" }) } as Response;
        }
    ...
    fireEvent.change(screen.getByPlaceholderText("New requirement"), { target: { value: "mfa-required" } });
    fireEvent.click(screen.getByRole("button", { name: "Add requirement" }));
    expect(await screen.findByText("Requirement added")).toBeInTheDocument();

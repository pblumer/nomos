# 013 - Dev- und Prod-Umgebung fuer MVP 0.1

## Zweck
Dieses Dokument beschreibt den einfachsten, kontrollierten Aufbau fuer Entwicklungs- und Produktionsumgebung in MVP 0.1. Ziel ist ein schneller Start mit wenig Betriebsaufwand und klarer Erweiterbarkeit.

## Leitprinzipien fuer die Umgebungen
- Einfachheit vor Vollausbau.
- Gleiche Runtime-Bausteine in Dev und Prod, damit Verhalten moeglichst konsistent bleibt.
- Git-first bleibt unveraendert: fachliche Artefakte liegen im Repository, nicht in einer primaeren Datenbank.
- Containerisierung fuer reproduzierbare Setups.
...
- lokale Arbeitsverzeichnisse als Volumes:
  - artefact workspace
  - temporaere git workspaces

Vorteile:
- Ein Kommando fuer Start und Stop.
- Weniger lokale Abhaengigkeitsprobleme.
- Gleiches Containerbild kann spaeter in Prod genutzt werden.

### 2) Prod-Umgebung (initial)
Empfehlung fuer MVP: Single-VM Deployment mit Docker Compose.

Bausteine:
- reverse proxy mit TLS (z. B. Caddy oder Traefik)
- frontend als statische Assets (Nginx Container oder direkt ueber Proxy)
- backend als FastAPI Container
- persistente Volumes fuer:
  - temporaere Workspace-Daten
  - Logs (falls lokal gesammelt)
- Git Provider als externes System (GitHub/GitLab)

Warum Single-VM fuer MVP:
- Schnellster Weg in stabilen Betrieb.
- Geringe Betriebskomplexitaet.
- Ausreichend fuer erwartete Last in MVP 0.1.

## Nicht-Ziele im MVP-Betrieb
- Kein Kubernetes-Zwang.
- Kein verteiltes Multi-Region Setup.
- Kein primaeres Datenbankcluster fuer Business-Artefakte.
- Kein vollstaendiges Enterprise-SSO als Blocker fuer den Start.

## Konfiguration pro Umgebung

### Gemeinsame Konfigurationsprinzipien
- Alle Umgebungswerte ueber ENV Variablen.
...
### Prod
- Multi-stage Docker Build fuer frontend und backend
- Versionierte Container-Tags pro Release (z. B. git sha)
- Deployment ueber compose pull + compose up -d
- Rollback ueber vorherigen Image-Tag

## Minimaler Betriebsablauf fuer Prod
1. CI baut und testet Images.
2. CI pusht Images in Registry.
3. Deployment-Pipeline aktualisiert compose auf Ziel-VM.
4. Healthcheck prueft API und UI Erreichbarkeit.
5. Bei Fehler: automatischer oder manueller Rollback auf vorherigen Tag.

## Sicherheitsbaseline fuer MVP
- TLS nur ueber reverse proxy.
- Secrets ausschliesslich ueber Secret Store oder .env auf Server (nicht im Git).
...
- Auditfaehigkeit ueber Git Historie und CI Logs.

## Observability baseline
- Strukturierte Backend-Logs (json oder klar formatierte text logs).
- Reverse-Proxy Access Logs.
- Einfache Uptime-Checks fuer /health Endpunkt.
- Fehlertracking kann spaeter ergaenzt werden (kein MVP-Blocker).

## Verzeichnisvorschlag fuer Betriebsartefakte
- /deploy
...
- Nutzt den bereits gewaehlten Stack direkt ohne Zusatzplattform.
- Trennt klar zwischen lokalem Arbeiten und stabilem Betrieb.
- Erlaubt spaetere Migration auf Kubernetes oder Managed Plattform ohne Neuschreiben der Anwendung.
- Bleibt konsistent mit ADR-0001 bis ADR-0004.

## Upgradepfad nach MVP
- Einfuehrung einer dedizierten Staging-Umgebung zwischen Dev und Prod.
...
## Akzeptanzkriterien fuer Phase-Start
- Entwickler koennen die Umgebung mit einem klaren Startkommando lokal hochfahren.
- Prod laeuft reproduzierbar auf einer frischen VM anhand dokumentierter Schritte.
